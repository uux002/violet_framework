...
public class ConfigManager {
    ...
    public static void LoadConfigData(string _confVoPackageName, string _path, bool _encrypted = false) {
    ...
    public static void LoadConfigDataFromByteArray(byte[] _data, bool _encrypted = false) {
    ...
    /// <returns></returns>
    public static TClass GetConfVO<TClass>(int id) where TClass : AbstractConfVO {
        ...
        if (ConfigManager._VOMap.TryGetValue(type.FullName, out table) && table.TryGetValue(id, out data)) {
            ...
            if (data is AbstractConfVO) {
                ...
                ConfigManager.fill(raw, table, (string[])data);
    ...
    /// <returns></returns>
    public static TClass[] GetConfVOList<TClass>() where TClass : AbstractConfVO {
        Type type = typeof(TClass);
        Dictionary<int, object> table;
            ...
            foreach (KeyValuePair<int, object> pair in table) {
                if (pair.Key >= 0) {
                    if (pair.Value is AbstractConfVO) {
                        ...
                        object raw = Activator.CreateInstance(type);
                        ConfigManager.fill(raw, table, (string[])pair.Value);
                        TClass vo = (TClass)raw;
    ...
    /// <returns></returns>
    public static TClass[] GetConfVOList<TClass>(params object[] args) where TClass : AbstractConfVO {
        Type type = typeof(TClass);
        TClass[] voList = ConfigManager.GetConfVOList<TClass>();
        List<TClass> list = new List<TClass>();
        //遍历属性表中的属性
        int i = 0;
    ...
    /// <param name="args"></param>
    /// <returns></returns>
    public static TClass[] GetConfVOListAnd<TClass>(params object[] args) where TClass : AbstractConfVO {
        Type type = typeof(TClass);
        TClass[] voList = ConfigManager.GetConfVOList<TClass>();
        List<TClass> list = new List<TClass>();
        //遍历属性表中的属性
    ...
    private static void InitializeCSV(string confVoPackageName, string csv) {
        ConfigManager._confVoPackageName = confVoPackageName;
        ...
        if (!string.IsNullOrEmpty(confVoPackageName)) {
            className = ConfigManager._confVoPackageName + "." + list[0][0];
        } else {
            className = list[0][0];
        }
        //string 
    ...
    //填充csv数据
    private static void fill(object vo, Dictionary<int, object> table, string[] data) {
    ...
    //设置对象属性
    private static void setValue(object vo, string attrName, object value) {
    ...
    //获取数组属性
    private static void getArrAttrNameAndIndex(string attrName, ref string arrAttrName, ref int arrIndex) {
        if (attrName.IndexOf('[') < 0) {
    ...
    //添加数组属性
    private static void addValue(object vo, string arrAttrName, int arrIndex, object value) {
    ...
    //设置内部类的属性
    private static void setInnerVoValue(object innerVo,
