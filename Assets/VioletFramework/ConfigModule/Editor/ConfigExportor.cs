...
namespace Libs.Editor
{
    //值对象类导出器
    public class ConfigExportor
    {
        private static string _packageName = "Fun.Data.Config";
        private static string _sourcePath = Application.dataPath + "/Data";
        private static string _targetPath = Application.dataPath + "War/Data/Config";
        ...
        public static void ExportConfig(string _csvSourcePath, string _configFilePath, bool encrypt = false)
        {
            ...
            {
                string txt = FileUtils.OpenTxtFile(fileList[i].FullName);
                bytes.WriteUTF(txt);
            }
            FileUtils.WriteBinaryFile(bytes.bytes, fullTargetPath);

            if (encrypt) {
                FunEngine.Crypto.DES.EncryptFile("-0q,cfma", "qp[qomvma'u'cf", fullTargetPath);
            }
                                               ...
                                               string targetPath)
        {
            ...
            {
                string csv = FileUtils.OpenTxtFile(csvList[i].FullName);
                List<string[]> datas = CSV.Read(csv);
                datas.RemoveAt(0);
                //List<string[]> datas = CSV.Read(csv);
                exportCode(ref datas);
            }
            Debug.Log(typeof(ConfigExportor) + " 值对象代码生成完毕");
        }
        ...
        private static void exportCode(ref List<string[]> datas)
        {
            ...
            FileUtils.WriteTxtFile(code, classFullPath);
            Debug.Log(typeof(ConfigExportor) + " 写入值对象代码:" + classFullPath);
        }
